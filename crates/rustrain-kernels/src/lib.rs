...
fn build_plugin() -> &'static RsPlugin {
    PluginBuilder::new("reference", env!("CARGO_PKG_VERSION"))
        ...
        // The third declaration is a *reduction*, not a routing: the down
        // projection splits its contraction dim (I) across tp, so the output is
        // a partial sum over tp and owes one all_reduce. Nothing in the
        // incoming layouts says so — the op's math does — which is exactly why
        // it is declared. The planner turns the declaration into a
        // `partial(sum, tp)` and the layout walk places the reduction at the
        // consumer that needs a replicated value.
        .collective(RsCollective {
            kind: RsCollectiveKind::ALL_REDUCE,
            group: RsGroupKind::TP,
            tensor_index: 10,
            on_side_stream: 0,
        }))
        .build()
