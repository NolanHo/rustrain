...
pub fn qwen_greedy_generate(
    ...
    for _ in 0..max_new_tokens {
        let logits = qwen3_forward_from_ids(&generated, weights, config)?;
...
pub(crate) fn qwen_greedy_generate_with_kind(
    ...
    let mut generated = input_ids.shallow_clone();
    for _ in 0..max_new_tokens {
        let logits = qwen3_forward_from_ids_with_kind(&generated, weights, config, compute_kind)?;
        let next_token = logits.i((0, -1)).argmax(-1, false).reshape([1, 1]);
        generated = Tensor::cat(&[&generated, &next_token], 1);
    }
    Ok(generated)
...
pub(crate) fn qwen_greedy_generate_with_lora(
    ...
    for _ in 0..max_new_tokens {
        let logits =
            qwen3_forward_from_ids_with_lora(&generated, weights, config, registry, compute_kind)?;
        let next_token = logits.i((0, -1)).argmax(-1, false).reshape([1, 1]);
        generated = Tensor::cat(&[&generated, &next_token], 1);
    }
    Ok(generated)
...
pub fn qwen_greedy_generate_with_cache(
    ...
    let mut generated = input_ids.shallow_clone();
    let (logits, mut cache) = qwen3_forward_with_cache(input_ids, weights, config, None)?;
    ...
    for step in 0..max_new_tokens {
        ...
        let (decode_logits, updated_cache) =
            qwen3_forward_with_cache(&next_token, weights, config, Some(cache))?;
        cache = updated_cache;
