...
pub fn train_qwen3_lora_sft_from_config(
    config: &Config,
    run_paths: &RunPaths,
) -> Result<QwenLoraSftTrainSummary> {
    if config.model.architecture != "qwen3_lora_sft" {
        bail!(
            "qwen3 LoRA SFT trainer expects architecture = qwen3_lora_sft, got {}",
            config.model.architecture
        );
    }
    if !matches!(config.train.device, RuntimeDevice::Cuda) {
        bail!("qwen3 LoRA SFT trainer requires device = cuda");
    }
    if config.parallel.data_parallel_size != 1 {
        bail!("qwen3 LoRA SFT trainer currently expects data_parallel_size = 1");
    }
    let model_path = config
        .model
        .model_path
        .as_ref()
        .context("qwen3 LoRA SFT trainer requires model.model_path")?;
    let model_path = resolve_qwen3_model_path(model_path)?;
    let data = config
        .data
        .as_ref()
        .context("qwen3 LoRA SFT trainer requires [data] instruction_jsonl")?;
    if data.kind != RuntimeDataKind::InstructionJsonl {
        bail!("qwen3 LoRA SFT trainer requires data.kind = instruction_jsonl");
    }
    if data.paths.is_empty() {
        bail!("qwen3 LoRA SFT trainer requires at least one data path");
    }
    let lora_config = QwenLoraConfig::from_runtime(
        config
            .lora
            .as_ref()
            .ok_or_else(|| anyhow!("qwen3_lora_sft requires [lora] config"))?,
    )?;
    let dtype = match config.train.dtype {
        ...
        rustrain_core::runtime::DType::Fp16 => {
            bail!("qwen3 LoRA SFT trainer does not support fp16 yet; use fp32 or bf16")
        }
    };
...
pub(crate) fn qwen3_lora_sft_train(
    model_path: &Path,
    adapter_output: &Path,
    checkpoint_dir: &Path,
    ...
    let initial_loss = qwen3_lora_sft_loss(
    ...
    .double_value(&[]);
    let initial_eval_loss = qwen3_lora_sft_loss(
    ...
    for step in 0..steps {
        ...
        let step_lr = qwen3_lora_sft_learning_rate(
            learning_rate,
        ...
        if qwen3_lora_sft_should_eval_step(step_number, eval_every) {
            let eval_loss = qwen3_lora_sft_loss(
                &eval_batch.input_ids,
                &eval_batch.target_mask,
                &weights,
                &lora_config,
    ...
    let final_loss = qwen3_lora_sft_loss(
        &initial_batch.input_ids,
        &initial_batch.target_mask,
        &weights,
        &lora_config,
    ...
    .double_value(&[]);
    let final_eval_loss = qwen3_lora_sft_loss(
        &eval_batch.input_ids,
        &eval_batch.target_mask,
        &weights,
        &lora_config,
    ...
    let reloaded_loss = qwen3_lora_sft_loss(
        &initial_batch.input_ids,
        &initial_batch.target_mask,
        &weights,
        &lora_config,
    ...
    let reloaded_eval_loss = qwen3_lora_sft_loss(
        &eval_batch.input_ids,
        &eval_batch.target_mask,
        &weights,
        &lora_config,
    ...
    let base_logits = qwen3_forward_from_ids_with_kind(
        &initial_batch.input_ids,
        &weights,
        &config,
        dtype.kind(),
    )?;
    let adapted_logits = qwen3_forward_from_ids_with_lora(
        &initial_batch.input_ids,
        &weights,
        &config,
        &registry,
    ...
    let reloaded_logits = qwen3_forward_from_ids_with_lora(
        &initial_batch.input_ids,
        &weights,
        &config,
        &reloaded,
    ...
    let merged_logits = qwen3_forward_from_ids_with_kind(
        &initial_batch.input_ids,
        &merged_weights,
        &config,
        dtype.kind(),
    ...
    let unmerged_logits = qwen3_forward_from_ids_with_kind(
        &initial_batch.input_ids,
        &unmerged_weights,
        &config,
        dtype.kind(),
...
pub(crate) fn qwen3_lora_sft_should_eval_step(step_number: usize, eval_every: u64) -> bool {
...
pub(crate) fn qwen3_lora_sft_loss(
    input_ids: &Tensor,
    target_mask: &Tensor,
    weights: &BTreeMap<String, Tensor>,
    lora_config: &QwenLoraConfig,
...
pub(crate) fn qwen_layer_lora_sft_loss(
    ...
    let base_output = qwen3_layer(&hidden, &layer, config);
    let target = lora_train_target(&base_output);
    let adapted = qwen3_layer_with_lora(&hidden, &layer, adapter, config);
...
pub(crate) fn qwen3_lora_sft_learning_rate(
...
mod tests {
    ...
    fn qwen_attention_lora_train_step_reduces_tiny_mse_and_reloads() {
        ...
        let initial_loss =
            qwen3_attention_lora_mse_loss(&input, &target, &layer, &adapter, &config)
                .double_value(&[]);
        let loss = qwen3_attention_lora_mse_loss(&input, &target, &layer, &adapter, &config);
        loss.backward();
        ...
        let final_loss = qwen3_attention_lora_mse_loss(&input, &target, &layer, &adapter, &config)
            .double_value(&[]);
        ...
        let reloaded =
            QwenAttentionLoraAdapter::load(&adapter_output).expect("adapter should reload");
        let reloaded_loss =
            qwen3_attention_lora_mse_loss(&input, &target, &layer, &reloaded, &config)
                .double_value(&[]);
    ...
    fn qwen_lora_registry_applies_all_projection_targets_to_layer() {
        ...
        let base_output = qwen3_layer(&input, &base_layer, &config);
        let zero_output = qwen3_layer_with_lora(
            &input,
            &base_layer,
            zero_registry.layer_adapter(0).expect("zero adapter"),
            &config,
        );
        let adapted_output = qwen3_layer_with_lora(
            &input,
            &base_layer,
        ...
        let merged_output = qwen3_layer(&input, &merged_layer, &config);
        assert!(
        ...
        let unmerged_output = qwen3_layer(&input, &unmerged_layer, &config);
        assert!(
    ...
    #[test]
    fn qwen3_lora_sft_resume_config_validation_checks_manifest_and_adapter() {
    ...
    fn qwen_lora_full_forward_and_generate_reload_parity() {
        ...
        let base_logits =
            qwen3_forward_from_ids(&input_ids, &weights, &config).expect("base forward should run");
        let adapted_logits =
            qwen3_forward_from_ids_with_lora(&input_ids, &weights, &config, &registry, Kind::Float)
        ...
        let reloaded = QwenLoraRegistry::load(&adapter_output).expect("registry should reload");
        let reloaded_logits =
            qwen3_forward_from_ids_with_lora(&input_ids, &weights, &config, &reloaded, Kind::Float)
        ...
        let merged_logits = qwen3_forward_from_ids(&input_ids, &merged_weights, &config)
        ...
        let unmerged_logits = qwen3_forward_from_ids(&input_ids, &unmerged_weights, &config)
    ...
    #[test]
    fn qwen3_lora_sft_eval_every_selects_periodic_steps() {
        assert!(!qwen3_lora_sft_should_eval_step(1, 0));
        assert!(qwen3_lora_sft_should_eval_step(1, 1));
        assert!(!qwen3_lora_sft_should_eval_step(1, 2));
        assert!(qwen3_lora_sft_should_eval_step(2, 2));
        assert!(qwen3_lora_sft_should_eval_step(4, 2));
    }
}
