...
pub(crate) fn resolve_qwen3_model_safetensors_path(model_safetensors: &Path) -> Result<PathBuf> {
    ...
    Ok(resolve_qwen3_model_path(model_path)?.join("model.safetensors"))
}

pub(crate) fn read_qwen3_runtime_config(path: &Path) -> Result<QwenRuntimeConfig> {
    let contents =
...
pub fn resolve_qwen3_model_path(model_path: &Path) -> Result<PathBuf> {
    if qwen3_model_path_is_complete(model_path) {
    ...
    candidates
        .into_iter()
        .rev()
        .find(|candidate| qwen3_model_path_is_complete(candidate))
        .ok_or_else(|| {
            anyhow!(
...
pub fn qwen3_forward_from_ids(
    ...
    qwen3_forward_from_ids_with_kind(input_ids, weights, config, Kind::Float)
}

pub(crate) fn qwen3_forward_from_ids_with_kind(
    input_ids: &Tensor,
    weights: &BTreeMap<String, Tensor>,
    config: &QwenRuntimeConfig,
    compute_kind: Kind,
...
pub(crate) fn qwen3_causal_lm_loss(
    ...
    qwen3_causal_lm_loss_with_kind(input_ids, weights, config, Kind::Float)
}

pub(crate) fn qwen3_causal_lm_loss_with_kind(
    input_ids: &Tensor,
    weights: &BTreeMap<String, Tensor>,
    config: &QwenRuntimeConfig,
    compute_kind: Kind,
) -> Result<Tensor> {
    let logits = qwen3_forward_from_ids_with_kind(input_ids, weights, config, compute_kind)?;
...
pub fn qwen3_attention(
    ...
    let q = input
        .linear::<&Tensor>(q_proj, None)
        .reshape([batch_size, seq_len, config.num_attention_heads, head_dim])
        .transpose(1, 2);
    let k = input
        .linear::<&Tensor>(k_proj, None)
        .reshape([batch_size, seq_len, config.num_key_value_heads, head_dim])
        .transpose(1, 2);
    let v = input
        .linear::<&Tensor>(v_proj, None)
        .reshape([batch_size, seq_len, config.num_key_value_heads, head_dim])
        .transpose(1, 2);
    let q = rms_norm(&q, q_norm, config.rms_norm_eps);
    let k = rms_norm(&k, k_norm, config.rms_norm_eps);
    ...
    let context = probs.matmul(&v).transpose(1, 2).reshape([
        batch_size,
        seq_len,
...
pub(crate) fn qwen3_attention_with_lora(
    input: &Tensor,
    weights: &QwenLayerWeights,
...
pub(crate) fn qwen3_layer_with_lora(
    input: &Tensor,
    weights: &QwenLayerWeights,
    adapter: &QwenAttentionLoraAdapter,
    config: &QwenRuntimeConfig,
    ...
    let attention_input =
        rms_norm(&input, &weights.input_norm, config.rms_norm_eps).to_kind(compute_kind);
    let attention_output = qwen3_attention_with_lora(&attention_input, weights, adapter, config);
    let after_attention = input + attention_output;
    let mlp_input = rms_norm(
        &after_attention,
        &weights.post_attention_norm,
...
pub(crate) fn qwen3_attention_lora_mse_loss(
    input: &Tensor,
    target: &Tensor,
    weights: &QwenLayerWeights,
    adapter: &QwenAttentionLoraAdapter,
    ...
    qwen3_attention_with_lora(input, weights, adapter, config).mse_loss(target, Reduction::Mean)
}
...
pub(crate) fn qwen3_tensor_i64_fingerprint(tensor: &Tensor) -> Result<String> {
    let values: Vec<i64> = Vec::<i64>::try_from(tensor.reshape([-1]).to_device(Device::Cpu))?;
    let mut hash = 0xcbf2_9ce4_8422_2325_u64;
    for value in values {
        qwen3_sft_hash_bytes(&mut hash, &value.to_le_bytes());
    }
    Ok(format!("{hash:016x}"))
}

#[allow(clippy::too_many_arguments)]
pub(crate) fn qwen3_attention_with_cache(
    input: &Tensor,
    q_proj: &Tensor,
    q_norm: &Tensor,
    k_proj: &Tensor,
    ...
    let context = probs.matmul(&v_for_attention).transpose(1, 2).reshape([
        batch_size,
        seq_len,
        config.num_attention_heads * head_dim,
    ]);
...
mod tests {
    ...
    #[test]
    fn qwen3_causal_lm_loss_is_finite_for_tiny_weights() {
    ...
    fn qwen_model_path_resolves_hf_hub_snapshot_when_legacy_dir_is_missing() {
        ...
        let resolved =
            resolve_qwen3_model_path(&legacy).expect("legacy path should resolve through HF hub");
    ...
    fn qwen_model_path_keeps_complete_configured_directory() {
        ...
        let resolved =
            resolve_qwen3_model_path(&model_path).expect("complete path should not be rewritten");
    ...
    fn qwen_model_path_reports_missing_hf_snapshot() {
        ...
        let error = match resolve_qwen3_model_path(&legacy) {
    ...
    fn qwen_model_safetensors_path_resolves_with_hf_hub_snapshot() {
        ...
        let resolved = resolve_qwen3_model_safetensors_path(&legacy_safetensors)
