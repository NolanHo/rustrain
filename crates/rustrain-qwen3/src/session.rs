...
pub fn train_qwen3_session_dp_from_config(config: &Config, _run_paths: &RunPaths) -> Result<()> {
    if config.model.architecture != "qwen3_trainable_session" {
        bail!(
            "qwen3 session trainer expects architecture = qwen3_trainable_session, got {}",
            config.model.architecture
        );
    }
    if !matches!(config.train.device, RuntimeDevice::Cuda) {
        bail!("qwen3 session trainer requires device = cuda");
    }
    if config.parallel.data_parallel_size != 2 {
        bail!("qwen3 session trainer currently expects data_parallel_size = 2");
    }
    let model_path = config
        .model
        .model_path
        .as_ref()
        .context("qwen3 session trainer requires model.model_path")?;
    let model_path = resolve_qwen3_model_path(model_path)?;
    ...
    let dtype = match config.train.dtype {
        ...
        rustrain_core::runtime::DType::Fp16 => {
            bail!("qwen3 session trainer does not support fp16 yet; use fp32 or bf16")
        }
    };
...
pub fn train_qwen3_session_tp_from_config(config: &Config, _run_paths: &RunPaths) -> Result<()> {
    if config.model.architecture != "qwen3_trainable_session" {
        bail!(
            "qwen3 session trainer expects architecture = qwen3_trainable_session, got {}",
            config.model.architecture
...
pub fn train_qwen3_session_single_from_config(
    config: &Config,
    run_paths: &RunPaths,
) -> Result<QwenFullTrainSmokeSummary> {
    if config.model.architecture != "qwen3_trainable_session" {
...
impl QwenTrainableSession {
    ...
    pub(crate) fn loss_value(&self) -> Result<f64> {
        Ok(qwen3_causal_lm_loss_with_kind(
            &self.input_ids,
            &self.weights,
            &self.config,
            self.compute_kind,
    ...
    pub(crate) fn loss_and_backward(&mut self) -> Result<f64> {
        ...
        let loss = qwen3_causal_lm_loss_with_kind(
            &self.input_ids,
            &self.weights,
            &self.config,
            self.compute_kind,
...
impl QwenAttentionDpSession {
    ...
    pub(crate) fn loss_tensor(&self) -> Tensor {
        let output = qwen3_attention(
            &self.input,
            &self.q_proj.to_kind(self.compute_kind),
            &self.q_norm.to_kind(self.compute_kind),
            &self.k_proj.to_kind(self.compute_kind),
            &self.k_norm.to_kind(self.compute_kind),
    ...
    pub(crate) fn parameters(&self) -> [(&'static str, &Tensor); 6] {
        [
            ("model.layers.0.self_attn.q_proj.weight", &self.q_proj),
            ("model.layers.0.self_attn.q_norm.weight", &self.q_norm),
            ("model.layers.0.self_attn.k_proj.weight", &self.k_proj),
            ("model.layers.0.self_attn.k_norm.weight", &self.k_norm),
            ("model.layers.0.self_attn.v_proj.weight", &self.v_proj),
            ("model.layers.0.self_attn.o_proj.weight", &self.o_proj),
        ]
    }

    pub(crate) fn parameters_mut(&mut self) -> [(&'static str, &mut Tensor); 6] {
        [
            ("model.layers.0.self_attn.q_proj.weight", &mut self.q_proj),
            ("model.layers.0.self_attn.q_norm.weight", &mut self.q_norm),
            ("model.layers.0.self_attn.k_proj.weight", &mut self.k_proj),
            ("model.layers.0.self_attn.k_norm.weight", &mut self.k_norm),
...
mod tests {
    ...
    fn representative_full_train_tensors_get_gradients_and_reload() {
        ...
        let initial_loss = qwen3_causal_lm_loss(&input_ids, &weights, &config)
            .expect("loss should run")
            .double_value(&[]);
        let loss = qwen3_causal_lm_loss(&input_ids, &weights, &config).expect("loss should run");
        loss.backward();
        ...
        let final_loss = qwen3_causal_lm_loss(&input_ids, &weights, &config)
            .expect("loss should run")
            .double_value(&[]);
        ...
        let reloaded_loss = qwen3_causal_lm_loss(&input_ids, &reloaded_weights, &config)
            .expect("loss should run")
            .double_value(&[]);
    ...
    #[test]
    fn qwen3_trainable_session_can_train_multiple_layers() {
        let config = QwenRuntimeConfig {
            num_hidden_layers: 2,
            num_attention_heads: 2,
            num_key_value_heads: 1,
        ...
        assert_eq!(step.artifacts.tensor_summaries.len(), 24);
    ...
    fn qwen_manifest_resume_reproduces_second_full_train_step() {
        ...
        let initial_loss = qwen3_causal_lm_loss(&input_ids, &continuous_weights, &config)
            .expect("loss should run")
            .double_value(&[]);
        let first_loss = qwen3_causal_lm_loss(&input_ids, &continuous_weights, &config)
            .expect("loss should run");
        first_loss.backward();
        ...
        let final_loss = qwen3_causal_lm_loss(&input_ids, &continuous_weights, &config)
            .expect("loss should run")
            .double_value(&[]);
        ...
        let resumed_loss = qwen3_causal_lm_loss(&input_ids, &resumed_weights, &config)
            .expect("loss should run")
            .double_value(&[]);
        assert!((final_loss - resumed_loss).abs() < 1e-6);

        continuous_registry.zero_grad();
        let continuous_second_loss = qwen3_causal_lm_loss(&input_ids, &continuous_weights, &config)
            .expect("loss should run");
        ...
        let resumed_second_loss =
            qwen3_causal_lm_loss(&input_ids, &resumed_weights, &config).expect("loss should run");
        resumed_second_loss.backward();
        resumed_registry
            .adamw_step(&mut resumed_weights, learning_rate, 2)
            .expect("resumed second step should apply");

        let continuous_after_second =
            qwen3_causal_lm_loss(&input_ids, &continuous_weights, &config)
                .expect("loss should run")
                .double_value(&[]);
        let resumed_after_second = qwen3_causal_lm_loss(&input_ids, &resumed_weights, &config)
            .expect("loss should run")
            .double_value(&[]);
    ...
    #[test]
    fn qwen3_trainable_session_trains_and_resumes_from_manifest() {
        let temp = tempfile::tempdir().expect("temp dir should be created");
