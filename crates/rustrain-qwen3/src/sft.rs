...
pub fn qwen_sft_streaming_batch_plan(
    ...
    for batch_index in 0..train_batch_count {
        ...
        batch_token_fingerprints.push(qwen3_tensor_i64_fingerprint(&streaming_batch.input_ids)?);
...
pub fn qwen_sft_arrow_batch_plan(
    ...
    for batch_index in 0..train_batch_count {
        ...
        batch_token_fingerprints.push(qwen3_tensor_i64_fingerprint(&batch.input_ids)?);
    }
...
pub(crate) fn qwen_combine_sft_fingerprints(
    ...
    qwen3_sft_hash_bytes(&mut hash, b"train");
    qwen3_sft_hash_bytes(&mut hash, train_fingerprint.as_bytes());
    qwen3_sft_hash_bytes(&mut hash, b"\0eval");
    qwen3_sft_hash_bytes(&mut hash, eval_fingerprint.as_bytes());
    for file in source_files {
        qwen3_sft_hash_bytes(&mut hash, b"\0path");
        qwen3_sft_hash_bytes(&mut hash, file.as_bytes());
    }
    format!("{hash:016x}")
}
...
pub(crate) fn qwen_sft_streaming_fingerprint_from_index(
    ...
    qwen_sft_hash_field_map(&mut hash, field_map);
    for file in source_files {
        qwen3_sft_hash_bytes(&mut hash, b"path");
        qwen3_sft_hash_bytes(&mut hash, file.as_bytes());
        qwen3_sft_hash_bytes(&mut hash, b"\0");
    }
...
pub(crate) fn qwen_sft_hash_source_file(hash: &mut u64, file: &str) {
    qwen3_sft_hash_bytes(hash, b"path");
    qwen3_sft_hash_bytes(hash, file.as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0");
}

pub(crate) fn qwen_sft_hash_field_map(hash: &mut u64, field_map: &QwenSftFieldMap) {
    qwen3_sft_hash_bytes(hash, b"field_map");
    qwen3_sft_hash_bytes(hash, field_map.instruction.as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0");
    qwen3_sft_hash_bytes(hash, field_map.input.as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0");
    qwen3_sft_hash_bytes(hash, field_map.response.as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0");
    if let Some(system) = &field_map.system {
        qwen3_sft_hash_bytes(hash, b"system");
        qwen3_sft_hash_bytes(hash, system.as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(chat_messages) = &field_map.chat_messages {
        qwen3_sft_hash_bytes(hash, b"chat_messages");
        qwen3_sft_hash_bytes(hash, chat_messages.as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    qwen3_sft_hash_bytes(hash, field_map.prompt_template.as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0");
    qwen3_sft_hash_bytes(hash, field_map.prompt_with_input_template.as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0");
    qwen3_sft_hash_bytes(
        hash,
        if field_map.trim_fields {
            b"trim".as_slice()
        } else {
    ...
    qwen3_sft_hash_bytes(hash, b"\0");
    qwen3_sft_hash_bytes(hash, b"min_response_chars");
    qwen3_sft_hash_bytes(hash, field_map.min_response_chars.to_string().as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0");
    if let Some(max_response_chars) = field_map.max_response_chars {
        qwen3_sft_hash_bytes(hash, b"max_response_chars");
        qwen3_sft_hash_bytes(hash, max_response_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if !field_map.field_replacements.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_replacements");
        for replacement in &field_map.field_replacements {
            qwen3_sft_hash_bytes(hash, format!("{:?}", replacement.field).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, replacement.pattern.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, replacement.replacement.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
        }
    }
    if !field_map.field_regex_replacements.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_regex_replacements");
    ...
    if field_map.normalize_whitespace {
        qwen3_sft_hash_bytes(hash, b"normalize_whitespace");
        qwen3_sft_hash_bytes(hash, b"true");
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if !field_map.field_defaults.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_defaults");
        for default in &field_map.field_defaults {
            qwen3_sft_hash_bytes(hash, format!("{:?}", default.field).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, default.value.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
        }
    }
    if !field_map.field_case_transforms.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_case_transforms");
        for transform in &field_map.field_case_transforms {
            qwen3_sft_hash_bytes(hash, format!("{:?}", transform.field).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, format!("{:?}", transform.case).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
        }
    }
    if !field_map.field_affixes.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_affixes");
        for affix in &field_map.field_affixes {
            qwen3_sft_hash_bytes(hash, format!("{:?}", affix.field).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, affix.prefix.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, affix.suffix.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
        }
    }
    if !field_map.field_strips.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_strips");
        for strip in &field_map.field_strips {
            qwen3_sft_hash_bytes(hash, format!("{:?}", strip.field).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, strip.prefix.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, strip.suffix.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
        }
    }
    if !field_map.field_splits.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_splits");
        for split in &field_map.field_splits {
            qwen3_sft_hash_bytes(hash, format!("{:?}", split.field).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, split.delimiter.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, format!("{:?}", split.side).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
        }
    }
    if !field_map.field_truncations.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_truncations");
        for truncation in &field_map.field_truncations {
            qwen3_sft_hash_bytes(hash, format!("{:?}", truncation.field).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, truncation.max_chars.to_string().as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
        }
    }
    if !field_map.field_transforms.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_transforms");
        for transform in &field_map.field_transforms {
            qwen3_sft_hash_bytes(hash, format!("{:?}", transform.field).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, format!("{:?}", transform.op).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, transform.value.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, transform.pattern.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, transform.replacement.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, transform.prefix.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, transform.suffix.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, transform.delimiter.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(
                hash,
                transform
    ...
    if !field_map.response_contains_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"response_contains_any");
        for needle in &field_map.response_contains_any {
            qwen3_sft_hash_bytes(hash, needle.as_bytes());
            qwen3_sft_hash_bytes(hash, b",");
        }
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if !field_map.response_excludes_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"response_excludes_any");
    ...
    if !field_map.instruction_contains_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"instruction_contains_any");
    ...
    if !field_map.instruction_excludes_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"instruction_excludes_any");
    ...
    if !field_map.input_contains_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"input_contains_any");
    ...
    if !field_map.input_excludes_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"input_excludes_any");
    ...
    if !field_map.field_regex_contains_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_regex_contains_any");
        for filter in &field_map.field_regex_contains_any {
            qwen3_sft_hash_bytes(hash, format!("{:?}", filter.field).as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, filter.pattern.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
        }
    }
    if !field_map.field_regex_excludes_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"field_regex_excludes_any");
    ...
    if !field_map.system_contains_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"system_contains_any");
    ...
    if !field_map.system_excludes_any.is_empty() {
        qwen3_sft_hash_bytes(hash, b"system_excludes_any");
    ...
    if let Some(min_instruction_chars) = field_map.min_instruction_chars {
        qwen3_sft_hash_bytes(hash, b"min_instruction_chars");
        qwen3_sft_hash_bytes(hash, min_instruction_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(max_instruction_chars) = field_map.max_instruction_chars {
        qwen3_sft_hash_bytes(hash, b"max_instruction_chars");
        qwen3_sft_hash_bytes(hash, max_instruction_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(min_input_chars) = field_map.min_input_chars {
        qwen3_sft_hash_bytes(hash, b"min_input_chars");
        qwen3_sft_hash_bytes(hash, min_input_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(max_input_chars) = field_map.max_input_chars {
        qwen3_sft_hash_bytes(hash, b"max_input_chars");
        qwen3_sft_hash_bytes(hash, max_input_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(min_system_chars) = field_map.min_system_chars {
        qwen3_sft_hash_bytes(hash, b"min_system_chars");
        qwen3_sft_hash_bytes(hash, min_system_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(max_system_chars) = field_map.max_system_chars {
        qwen3_sft_hash_bytes(hash, b"max_system_chars");
        qwen3_sft_hash_bytes(hash, max_system_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(min_prompt_chars) = field_map.min_prompt_chars {
        qwen3_sft_hash_bytes(hash, b"min_prompt_chars");
        qwen3_sft_hash_bytes(hash, min_prompt_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(max_prompt_chars) = field_map.max_prompt_chars {
        qwen3_sft_hash_bytes(hash, b"max_prompt_chars");
        qwen3_sft_hash_bytes(hash, max_prompt_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(min_sample_chars) = field_map.min_sample_chars {
        qwen3_sft_hash_bytes(hash, b"min_sample_chars");
        qwen3_sft_hash_bytes(hash, min_sample_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if let Some(max_sample_chars) = field_map.max_sample_chars {
        qwen3_sft_hash_bytes(hash, b"max_sample_chars");
        qwen3_sft_hash_bytes(hash, max_sample_chars.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if field_map.dedupe_samples {
        qwen3_sft_hash_bytes(hash, b"dedupe_samples");
        qwen3_sft_hash_bytes(hash, b"true");
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    if field_map.skip_invalid_records {
        qwen3_sft_hash_bytes(hash, b"skip_invalid_records");
        qwen3_sft_hash_bytes(hash, b"true");
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    qwen3_sft_hash_bytes(hash, b"source_weights");
    for source_weight in &field_map.source_weights {
        qwen3_sft_hash_bytes(hash, source_weight.to_string().as_bytes());
        qwen3_sft_hash_bytes(hash, b",");
    }
    qwen3_sft_hash_bytes(hash, b"\0");
    if !field_map.source_max_samples.is_empty() {
        qwen3_sft_hash_bytes(hash, b"source_max_samples");
        for source_limit in &field_map.source_max_samples {
            qwen3_sft_hash_bytes(hash, source_limit.to_string().as_bytes());
            qwen3_sft_hash_bytes(hash, b",");
        }
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    ...
    if !field_map.external_metadata.is_empty() {
        qwen3_sft_hash_bytes(hash, b"external_metadata");
        for metadata in &field_map.external_metadata {
            qwen3_sft_hash_bytes(hash, metadata.path.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
            qwen3_sft_hash_bytes(hash, metadata.contents.as_bytes());
            qwen3_sft_hash_bytes(hash, b"\0");
        }
    }
}
...
pub(crate) fn qwen_sft_hash_source_field_overrides(
    ...
    qwen3_sft_hash_bytes(hash, label);
    for value in values {
        qwen3_sft_hash_bytes(hash, value.as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
}
...
pub(crate) fn qwen_sft_hash_example(
    ...
    if include_system {
        qwen3_sft_hash_bytes(hash, b"system");
        qwen3_sft_hash_bytes(hash, example.system.as_bytes());
        qwen3_sft_hash_bytes(hash, b"\0");
    }
    qwen3_sft_hash_bytes(hash, b"instruction");
    qwen3_sft_hash_bytes(hash, example.instruction.as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0input");
    qwen3_sft_hash_bytes(hash, example.input.as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0response");
    qwen3_sft_hash_bytes(hash, example.response.as_bytes());
    qwen3_sft_hash_bytes(hash, b"\0");
}

pub(crate) fn qwen3_sft_hash_bytes(hash: &mut u64, bytes: &[u8]) {
    for byte in bytes {
        *hash ^= u64::from(*byte);
...
mod tests {
    use super::*;
    use crate::qwen3_module::test_utils::*;
