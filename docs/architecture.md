...
4. **算子声明的通信必须被兑现**（I-3 的第二条路）：声明 `ALL_REDUCE` 的算子让计划器把该张量标成
   `partial`，并在第一个需要完整值的消费者处插入归约；声明 `ALL_TO_ALL` / `ALL_GATHER` 等计划器
   还不能表达的种类时，只要声明的组度数 > 1 就**拒绝编译**（点名算子、种类、组、张量）——
   "声明了通信但计划里没有"是这个仓库最不能接受的形态。度数为 1 的组是恒等。
5. **规则不得按算子名查框架侧的表** —— 那会把 T2 泄漏成 T3。规则要作为**描述符里的声明**
