...
- JSONL SFT trainer summaries/logs expose `streaming_index_cache_path`,
  ...
  `data.trim_fields` and `data.min_response_chars`, so stale caches from a
  different external schema, prompt format, normalization policy, or response
  filtering policy are rejected.
