...
**已知 ABI 缺口（P3 的前置条件之二）：逐输入可导性。**
`rs_op_desc` 没有说明"哪些输入需要梯度"。而这不是细节：
`embedding` 的 indices 不可导、`linear` 的 bias 可导、`gather`/`scatter` 的索引不可导、
`cross_entropy` 的 target 不可导。没有这个信息，推导器无法知道该给哪个输入发伴随，
只能猜 —— 而猜错的表现是**静默错误的梯度**，比报错糟得多。
需要给 ABI 加一个输入掩码（或让 VJP 规则声明它消费哪些输入），并在装载时校验。
