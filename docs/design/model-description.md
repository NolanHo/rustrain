...
| 2 | 声明的切分怎么到 instantiate | **以符号轴名随描述走，不以 bit 随 plan 走**。展开层额外暴露 `DeclaredAxes`（slot 名 → 逻辑 dim → 轴名列表），`instantiate` 把它作为**输入**：plan 因此保持拓扑无关（digest 不会因为描述声明了切分而变），而 §0 的箭头"描述 × 拓扑 → plan"两个操作数都看得见。<br>轴名在 mesh 里不存在 → `GroupUnavailable`（与"掩码位越界"同一个错误）；`tp=1` 是 size-1 组，合法 |
...
| 4a. **位置常量（D4 未做）** | D4 只做到 1–3 与 5；位置常量（flat QKV 的通道偏移、CP 的序列偏移、本地专家范围）推迟到 **D5** —— 数值对齐前必须先有它们。`instantiate` 里留注释指到这里，不假装做了 |
