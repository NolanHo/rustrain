...
### 附二：算子声明的集合通信由计划器兑现（D6 落地）

有些算子的数学**看不见**在操作数里，通信只能由算子自己声明（`rs_op_desc::collectives`）：
`moe_layer` 声明的三项就是全部形态的代表。

| 声明 | 计划器现在做什么 |
|---|---|
| `ALL_REDUCE {tp}`（输出上） | 把该张量标成 `partial(sum, tp)`，剩下的交给既有的布局传播：**在第一个需要完整值的消费者处**插入那次归约。声明说的是"什么是部分的"，插在哪由代数决定 —— 两条都能发 all-reduce 的机制迟早会互相矛盾 |
| `ALL_TO_ALL {tp, ep}`（dispatch/combine） | **两个 ep 度数为 1** 时是恒等（没有专家被分布，路由什么也不用搬），跳过并在编译计划的 warnings 里记一行理由；**度数 > 1** 时计划器还没有表达，**按名字拒绝**（`l1.compile` fail，点名算子、种类、组、张量）—— 绝不带着缺失的通信跑 |
| `ALL_GATHER {cp}`（GDN 的状态） | 同上：`cp=1` 恒等；`cp>1` 拒绝 |

拒绝而不是警告，是因为"声明了通信但计划里没有"正是这个仓库最不愿接受的形态：错值不报错。
EP 与 CP 的落地（真正接线 all_to_all / all_gather）因此变成了两个具体的、有门禁的下一步。
