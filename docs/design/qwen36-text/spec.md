...
**由此得到的两个真正的杠杆**：
1. **分块流式**（不需要任何裁定）：不要把每个权重物化成一整个 `Vec<f32>` 再拷贝，而是按块
   （如 8 MiB）读→展宽→写设备，宿主流量从 ~136 GiB 降到 ~65 GiB/rank，预计把 `write` 段
   从 7.3–8.6 s 压到 3 s 量级。
2. **bf16 变体**（精度决定，见下）：直接把 bf16 权重写到设备，字节数再减半，两项叠加后可望把
   加载压到 2–3 s/rank。
