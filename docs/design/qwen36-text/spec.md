...
| | 改前 | 改后（三个提交依次落地） |
|---|---|---|
| 一次 sweep（baseline + tp=2） | **22 m 43 s** | **58–65 s**（23×；最近一次 1 m 01.8 s） |
...
**回写已经是最后的大头（改完才看见）**：加载墙钟 22.6 s 里，调用线程在 `write_f32` 里真正花掉
**18.3 s**（142 GB pageable 源 → ~7.8 GB/s），worker 的 fill 是并行的、`read` 也在并行 —— 也就是说
