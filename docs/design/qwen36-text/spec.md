...
**残留成本（已量化，下一步优化）**：组建立（`ncclCommInitRank` 握手）本身固定约 1.1 s，
叠加到达偏差后单次 1.1–2.6 s，全部落在"前向墙钟"里——占一次 tp=2 调试周期（约 16–19 s，含
9 s 加载）的 7–15%。把通信器建立**并行到加载窗口里**（加载 9 s 是 I/O 密集，通信器建立是纯等待）
可以把它从关键路径上拿掉，代价是让 NCCL 后端的 comm 缓存变成可跨线程移交（`&mut self` →
内部可变 + `Comm` 的 Send 语义），这是下一轮的事，本轮先把它变成可测量的数字。
