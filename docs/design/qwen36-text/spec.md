...
**剩下那 5 GB/s 的拷贝：已缩小范围，仍未解决**。一批对照测量把它从"平台极限"里排除了：

| 测量（同一宿主） | 结果 |
|---|---|
| 一个进程、8 MiB 块、源在 L3（8 MiB 循环） | **15.2–16.7 GB/s** |
| 源 256 MiB / 1 GiB 循环（DRAM 冷） | 11.5 / 9.8 GB/s |
| **四个进程各占一张卡、同时各拷 34.4 GiB** | 每进程 **15.1–16.8 GB/s**（合计 ~65 GB/s，没有 PCIe 聚合上限） |
| 四个进程各 16 线程同时读 31 GiB 页缓存 | 每进程 27–30 GB/s（合计 ~120 GB/s） |
| 上述读与"单卡拷贝"同时进行 | 拷贝仍是 16.4 GB/s（**不是内存/IO 争用**） |
| 真实加载进行时，另起一个进程做同样的拷贝 | 9.8 GB/s（**也不是被加载进程挤掉**） |
| 真实加载里的写者（`write_seconds`） | **约 5 GB/s** |

即：同一个驱动调用、同样的块大小，独立进程能到 10–16 GB/s，而加载进程里的写者只有 5 GB/s，
且 8 MiB 与 32 MiB 块没有区别（不是每次调用的固定开销）。已经排除：平台 PCIe 上限、文件系统、
页缓存读取争用、进程间争用、块大小。下一轮要在写者内部做细分测量（每块尺寸直方图 + 每块耗时，
或试"把块直接从池缓冲异步排队"），再看是目的地址分散、还是驱动在同步路径上的行为。

**杠杆二（待裁定）：bf16 变体**——直接写 bf16 权重，字节数减半；在杠杆一已经落地的基础上，
这是唯一能确定把拷贝段再砍一半的动作。
...
**② 之后的一个可选精度决定（同样待裁定，不是阻塞项）**：ATen 插件今天只发布
`cuda.aten.f32` 一个变体，所以 `run` 把 bf16 权重**加宽成 f32** 执行：tp=4 每 rank 权重 34.6 GiB、
峰值 35.8 GiB、加载里的设备回写 6.8 s；tp=8 是 18.3 / 19.3 GiB。加一个 `cuda.aten.bf16` 变体
（ATen composition，不是新 kernel）可以把这三项都减半（tp=4 → 17.3 GiB / ~18 GiB / ~3.4 s），
前向在长序列上还会用到 bf16 tensor core；代价是数值基准要重定（HF 参考本身就是 bf16，
所以 `max|diff|` 的界有机会更紧而不是更松）。它属于"精度是配置"的范畴，我不擅自改。
