...
| expert parallel | 已实现子集 | 默认 routed-output all-reduce；gated variable-split A2A 已验证 fixed-LoRA 和 native dynamic-LoRA data sharding。`QWEN36_MOE_SHARED_EXPERT_OVERLAP=1` 提供严格受限的 packed sharded-A2A 实验路径并通过 H20 smoke，但首轮 A/B p50 慢 `3.4%`、显存多约 `0.13 GiB`，默认关闭；GPU-only split planning、有效异步 overlap 和 DeepEP backend 仍未实现 |
