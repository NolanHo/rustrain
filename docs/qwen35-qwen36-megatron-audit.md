...
本轮新增 H20 ABI1 CP2 GDN dynamic-LoRA smoke：两个 selected tenant 使用不同有效 token 数，CP2 与单卡完整序列参考的 aggregate/per-tenant loss 差均为 `0`，LoRA 参数最大差 `9.88e-4`，Adam m/v 最大差 `1.53e-6` / `1.57e-10`；第三个未选 tenant 的参数、m/v 与 optimizer step 均逐位不变。实现将 CP local hidden 先 gather 后计算 CE，再由 autograd/手动 checkpoint backward 切回 local gradient，并在 dynamic grouped gradient sync 中加入 CP all-reduce；非 grouped dynamic CP 继续 fail-closed。
