...
PP 会把层分到不同 stage 并使用 1F1B 等调度；CP 会在序列和 attention state 上做跨 rank 通信。ABI26 已把 PP/CP communicator 纳入 `TrainingContext` 和进程级缓存，并通过五轴 size consensus 防止 rank 执行不同的 split 序列；4 卡 CP2 x PP2 smoke 的正交组全局 max 为 `3.0`。NCCL 文件 rendezvous 按 generation 隔离：launcher 使用共享 `RUSTRAIN_LAUNCH_OUTPUT_DIR`，直接多节点任务必须提供唯一 `RUSTRAIN_NCCL_RUN_ID` 和共享 `RUSTRAIN_NCCL_SYNC_DIR`。当前模型仍在每个进程持有并执行完整层栈，因此所有非单例 PP/CP train/eval 入口在读取输入前拒绝执行。
