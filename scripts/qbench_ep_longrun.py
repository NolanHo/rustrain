#!/usr/bin/env python3
"""EP=4 long-run benchmark — loss convergence, overfit, stability.

Tests: 1, 8, 32 adapters × 50 steps each.
Prints loss per step to observe convergence trends.
"""
...
def make_cfg(sid):
    ...
    return "\n".join([
        ...
        "[train]", "max_steps=1000", 'backend="tch"',
...
def setup_session(sid, n_adp, lora_rank=16):
    """Create session, load model, init LoRA, add adapters."""
...
def run_steps(sid, n_steps):
    """Run n_steps train_step calls, return (losses, times)."""
    ids = [1]*SEQ
    mask = [0]*20 + [1]*(SEQ-20)
    payload = {
    ...
    for s in range(n_steps):
        ...
        # Print every 5 steps
        if s % 5 == 0 or s == n_steps - 1:
            recent_avg = sum(times[max(0,s-4):s+1])/min(5, s+1)
            print(f"  step {s:3d}: loss={loss:.6f}  time={times[-1]:.0f}ms  (avg5={recent_avg:.0f}ms)")
        sys.stdout.flush()
    return losses, times
...
def bench(n_adp, n_steps):
    ...
    print(f"\n{'='*60}")
    print(f"  {n_adp} adapters × {n_steps} steps")
    print(f"{'='*60}")
    ...
    return {"n": n_adp, "losses": losses, "times": times, "avg": avg_t,
            "min": min_t, "max": max_t, "p50": p50, "p95": p95}
