...
for i, resp in enumerate(r):
    if resp.status_code != 200:
        ...
        exit(1)
print("init_lora OK, adding 8 adapters...")
...
for i in range(8):
    ...
    for j, resp in enumerate(r):
        if resp.status_code != 200:
            ...
            exit(1)
print("8 adapters added, running train_step...")
...
for i, resp in enumerate(r):
    if resp.status_code == 200:
        print(f"warmup rank {i}: loss={resp.json().get('loss','?')}")
    else:
        print(f"warmup rank {i}: FAIL {resp.text[:200]}")
...
for i, resp in enumerate(r):
    if resp.status_code == 200:
        print(f"step rank {i}: loss={resp.json().get('loss','?')}")
    else:
        print(f"step rank {i}: FAIL {resp.text[:200]}")
