#!/usr/bin/env python3
"""Test EP=4 with 8 adapters, lr=0 (no Adam update), 3 steps.
If all steps succeed, Adam CUDA kernel is the culprit.
If step 2 still crashes, the issue is from NCCL or forward."""
...
cfg = "\n".join([
    ...
    "[train]", "max_steps=3", 'backend="tch"',
    "micro_batch_size=1", "global_batch_size=1",
    "gradient_accumulation_steps=1", "learning_rate=0.0",  # lr=0: no Adam update
...
for step in range(3):
    ...
    for i, resp in enumerate(r):
        if resp.status_code == 200:
            print(f"step {step} rank {i}: loss={resp.json().get('loss','?')}")
        else:
            print(f"step {step} rank {i}: FAIL {resp.text[:200]}")
            ok = False
