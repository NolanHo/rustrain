...
BASE_CONFIG="${RUSTRAIN_QWEN_LORA_SFT_TRAINER_INDEX_CACHE_CONFIG:-configs/qwen_lora_sft_max_samples.toml}"
RUN_DIR="$(mktemp -d)"
CONFIG="${RUN_DIR}/lora-sft-index-cache.toml"
CACHE="${RUN_DIR}/qwen-lora-sft-offset-index.json"
...
for context, values in [("first", first), ("second", second)]:
    ...
    if missing:
        raise SystemExit(f"{context} LoRA SFT trainer run is missing fields: {missing}")
...
print(
    "qwen_lora_sft_trainer_index_cache_verified: "
    f"cache={cache_path} "
