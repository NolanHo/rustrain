...
OUTPUT_DIR="${RUSTRAIN_QWEN_TP_ATTENTION_OUTPUT_DIR:-/tmp/rustrain-runs/qwen-tp-attention-verify-$$}"
MODEL_PATH="${RUSTRAIN_QWEN_TP_ATTENTION_MODEL_PATH:-/vePFS-Mindverse/share/huggingface/Qwen2.5-0.5B-Instruct}"
...
summaries = sorted(rank_dir.glob("qwen-tp-attention-rank-*.json"))
if len(summaries) != 2:
    raise SystemExit(f"expected 2 TP attention rank summaries under {rank_dir}, found {len(summaries)}")
...
print(json.dumps({"qwen_tp_attention_verified": evidence, "assigned_cuda_visible_devices": assigned}, indent=2))
PY
