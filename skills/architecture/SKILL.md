...
❌ 把 rank 序号折进**本地**缓冲区的偏移 → 本地（已分片的）缓冲区没有成员轴：成员轴只存在于
                                     gather/scatter 的目标形状里，`all_gather` 的 send 与
                                     `reduce_scatter` 的 recv 的选块由 NCCL 自己按 rank 完成
...
3. **融合体声明的 `collectives` 必须等于 planner 在它的 expansion 上会插入的集合。**
   ...
   **声明是要被兑现的，不是文档**（D6 落地）：算子声明的 `ALL_REDUCE` 把该张量标成 `partial`，
   计划器在第一个需要完整值的消费者处插入归约；其余种类（`ALL_TO_ALL` / `ALL_GATHER` / …）在
   声明的组的度数 > 1 时**被计划器拒绝**（`l1.compile` fail，点名算子/种类/组/张量），因为计划里
   会缺一次它的数学欠下的通信。度数为 1 的组是恒等，跳过即可（ep=1 时没有专家被分布，路由不用搬）。
   加一个声明之前先回答：**这次通信在哪些 rank 之间真的需要发生，度数 1 时为什么是恒等。**
...
- **同一语义的两个实现，一个的绿灯不是另一个的证据。** 实测（D6 首次 tp=2 端到端）：NCCL 后端的
  `all_gather` 把 rank 序号折进了**本地**缓冲区一侧的偏移，非 root 的每个数据块都平移了一整块
  （最后一块还会读过本地缓冲区尾巴），而 root 自己那一半**逐元素正确** —— host reference 后端是另一个
  实现，写法正确、测试全绿，因此**没有一个测试看见这个缺陷**，它只在真多卡数值对比里现形。
  凡是**不需要 GPU 就能钉住**的那部分（偏移算术、组播顺序、形状增量）必须有一条内存里的模拟测试，
  并且**执行路径与测试共用同一组偏移函数**（`nccl.rs` 的 `all_gather_offsets` / `reduce_scatter_offsets`），
  否则测试会随着实现一起漂移。
...
- **启动路径必须分相计时，否则"慢"无法归因。** 实测一次真实加载 614 s 里 **92% 是宿主 CPU**
  （420 s 的 `transpose` 逐元素下标算术 + 74 s 的 slice + 71 s 的 widen），磁盘只有 34 s；只看总时长
  会去优化错的那一半。加载/编译这类一次性成本要在 metrics 里留下"读了多少字节、去重后多少、每一相多少秒"
  —— `checkpoint_bytes_read` 曾经把 `split` 段的**重复读**算成真实字节（873 个 pair / 712 个张量，
  +64.8%），一个数字掩盖了两个缺陷。
- **搬运路径重写的验收判据是"逐位相同"。** 加载器只决定"同样的值怎么进 slot"，不决定值本身：改完后
  rank 0 的 `logits` 与全部 hidden 必须与改前 `array_equal`（本次 tp=2 实测 `max|d| 0.0`，digest 不变）。
  "跑得通 + 判据 PASS" 太弱 —— 一个把行/列搞反但量级相同的加载器也能 PASS。价值相同的一份"旧实现"
  不要删掉：把它 `#[cfg(test)]` 降级成参考实现，用一条覆盖全部操作形态的链做逐元素差分测试。
