...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
struct QwenSftArrowRawSampleIndex {
    path: String,
    row_index: usize,
    global_index: usize,
}
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct QwenSftArrowSourceIndex {
    samples: Vec<QwenSftArrowRawSampleIndex>,
}
...
#[derive(Debug, Serialize, Deserialize)]
struct QwenSftArrowSourceIndexCache {
    format: String,
    paths: Vec<String>,
    source_files: Vec<QwenSftStreamingSourceFileMetadata>,
    max_samples: Option<usize>,
...
#[derive(Debug)]
struct QwenSftArrowSourceIndexLoad {
    index: QwenSftArrowSourceIndex,
    summary: Option<QwenSftStreamingSourceSummary>,
    cache_hit: bool,
    cache_written: bool,
...
struct QwenSftArrowStreamingTokenWindow {
    samples: Vec<QwenSftTokenSample>,
    raw_samples_read: usize,
    source_index_cache_hit: bool,
    source_index_cache_written: bool,
...
#[derive(Debug, Serialize)]
struct QwenSftArrowSourceSummary {
    input: String,
    arrow_ipc_format: String,
    limit: usize,
    source_rows: usize,
    source_rows_exact: bool,
    columns: Vec<String>,
...
fn qwen_sft_arrow_validate_config_scope(
...
fn qwen_sft_arrow_streaming_data_plan(
    config_path: &Path,
    config: &Config,
    data_config: &RuntimeDataConfig,
    world_size: usize,
...
fn qwen_sft_arrow_streaming_batch_plan(
    config_path: &Path,
    config: &Config,
    data_config: &RuntimeDataConfig,
    model_path: &Path,
    ...
    qwen_sft_arrow_validate_config_scope(data_config, "qwen SFT Arrow streaming batch plan")?;
    let tokenizer = Tokenizer::from_file(model_path.join("tokenizer.json"))
        .map_err(|error| anyhow!("failed to load tokenizer: {error}"))?;
    let field_map = QwenSftFieldMap::from_runtime_data(data_config)?;
...
fn qwen_sft_arrow_source_summary_from_ipc(
    path: &Path,
    limit: usize,
    field_map: &QwenSftFieldMap,
...
fn qwen_sft_arrow_source_summary_from_paths(
    paths: &[PathBuf],
    max_samples: Option<usize>,
    field_map: &QwenSftFieldMap,
) -> Result<QwenSftStreamingSourceSummary> {
...
fn qwen_sft_arrow_source_index_with_cache(
    paths: &[PathBuf],
    max_samples: Option<usize>,
    cache_path: Option<&Path>,
    field_map: &QwenSftFieldMap,
...
fn qwen_sft_single_file_source_metadata(
    paths: &[PathBuf],
) -> Result<Vec<QwenSftStreamingSourceFileMetadata>> {
    let mut source_files = Vec::new();
    for path in paths {
...
fn qwen_sft_arrow_streaming_token_window(
    tokenizer: &Tokenizer,
    paths: &[PathBuf],
    eval_paths: &[PathBuf],
    max_samples: Option<usize>,
...
fn qwen_sft_arrow_streaming_dataset_plan(
    tokenizer: &Tokenizer,
    data_config: &RuntimeDataConfig,
    seed: u64,
    data_cursor_start: usize,
    ...
    qwen_sft_arrow_validate_config_scope(
        data_config,
        "qwen trainable session instruction_arrow streaming runtime",
    )?;
    ...
    let train_summary = qwen_sft_arrow_source_summary_from_paths(
        &data_config.paths,
        data_config.max_samples,
        field_map,
    )?;
...
fn qwen_sft_arrow_examples_by_raw_indices(
...
fn qwen_sft_source_max_samples(
    path_count: usize,
    field_map: &QwenSftFieldMap,
...
fn qwen_apply_field_replacements(record: &mut QwenSftRecord, replacements: &[FieldReplacement]) {
...
fn qwen_apply_field_split(value: &str, split: &FieldSplit) -> String {
...
fn qwen_sft_string_contains_any_filter_passes(value: &str, needles: &[String]) -> bool {
